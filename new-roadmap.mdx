...
## Day 17 - Hypothesis Testing & Statistical Inference
...
Code Challenge (≈ 30 min)
  • `ttest_demo.py`
     – Perform a one-sample t-test on Iris sepal width
     – Perform a two-sample t-test between two Iris species
...
## Day 18 – Scikit-learn Fundamentals
...
Build (≈ 1 h)
  • `iris_classifier.ipynb`
     ...
     – Train a classifier
     – Save the trained model
  • Commit notebook, model, and README updates
```

---

## Day 19 – Supervised ML Algorithms & Model Evaluation
...
Build (≈ 1 h)
  • `titanic_prediction.ipynb`
     ...
     – Save the best model
  • Commit notebook, trained model, and evaluation report
```

---

## Day 20 – Unsupervised Learning, Feature Engineering & Hyperparameter Tuning
...
Build (≈ 1 h)
  • `customer_segmentation.ipynb`
     – Engineer features
     – Build customer clusters
     – Tune model parameters
     – Visualize results
  • Commit notebook and plots
```
...
Build (≈ 1 h)
  • Commit `housing_pipeline.ipynb`
  • Save the trained model
  • Add evaluation plots and README summary
```
...
Code Challenge (≈ 30 min)
  • `neuron_demo.py` – implement a single neuron from scratch 
  • Calculate weighted sum and bias 
  • Apply ReLU activation 
  • Calculate prediction error 
  • Update weights using a simple gradient step

Build (≈ 1 h)
  • `nn_theory.md` – explain the neural network training process 
  • Include a diagram for: Input → Forward Pass → Loss → Backpropagation → Weight Update 
  • Commit `neuron_demo.py` and `nn_theory.md`
```
